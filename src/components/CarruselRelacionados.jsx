...
export default function CarruselRelacionados({ titulo, items, tipo, infinito = false }) {
  const scrollRef = useRef(null);

  if (!items || items.length === 0) return null;

  const lista = infinito ? [...items, ...items] : items;
  ...
  const track = (
    <div className="carrusel-rel-track" ref={scrollRef}>
      {lista.map((it, i) => (
        <Link key={`${it.id}-${i}`} to={`/${it.tipo || tipo}/${it.id}`} className="carrusel-rel-item">
          {it.imagen
  ...
  return (
    <div className={`carrusel-rel ${infinito ? "infinito" : ""}`}>
      ...
      {infinito
        ? <div className="carrusel-rel-viewport">{track}</div>
        : track}
    </div>
  );
}
